...
const ProductCheckout: React.FC = () => {
  ...
  const fetchWalletBalance = async () => {
    try {
      ...
      if (data.success && data.user && data.user.wallet) {
          ...
          <CardContent className="p-8 text-center">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6 text-left">
              ...
              <p className="text-sm text-green-700">
                ...
                Derinlik Havuzu (%15): <strong>${Math.round(product.price * 0.15)}</strong><br />
                Monoline Havuzu (%10): <strong>${Math.round(product.price * 0.10)}</strong><br />
                Sistem Fonu (%50): <strong>${Math.round(product.price * 0.50)}</strong>
              </p>
            </div>
  ...
  // Product specific distribution (25% Sponsor, 15% Depth, 10% Monoline, 50% Company)
  const directSponsorBonus = subtotal * 0.25;
  const depthBonus = subtotal * 0.15;
  const monolinePoolBonus = subtotal * 0.10;
  const systemFund = subtotal * 0.50;
            ...
            <CardContent className="space-y-4">
              ...
              <div className="border-t pt-4 space-y-3">
                ...
                <div className="text-xs text-muted-foreground space-y-1">
                  ...
                  <p>• Derinlik Havuzu: ${depthBonus.toFixed(2)} (%15)</p>
                  <p>• Monoline Havuzu: ${monolinePoolBonus.toFixed(2)} (%10)</p>
            ...
            <CardContent className="space-y-6">
                  ...
                  {buyerId && (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <div className="flex-1">
                          <h4 className="font-medium text-purple-900">E-Cüzdan Bakiyesi</h4>
                          <p className="text-sm text-purple-700">Kazandığınız komisyon bakiyelerini kullanarak anında alışveriş yapın.</p>
                          <p className="text-xs font-bold text-purple-900 mt-1">
                            Mevcut Bakiyeniz: {walletBalance !== null ? `$${walletBalance.toFixed(2)}` : "Yükleniyor..."}
                          </p>
                          {walletBalance !== null && walletBalance < totalAmount && (
                            <p className="text-xs text-red-500 mt-1 font-semibold">
                              ⚠️ Yetersiz Bakiye (Gereken: ${totalAmount})
                            </p>
                          )}
                        </div>
              ...
              <div className="bg-gradient-to-r from-primary/10 to-spiritual-purple/10 rounded-lg p-4">
                ...
                <div className="text-sm space-y-1">
                  ...
                  <p>• Derinlik Havuzu (%15): <strong>${depthBonus.toFixed(2)}</strong></p>
                  <p>• Monoline Havuzu (%10): <strong>${monolinePoolBonus.toFixed(2)}</strong></p>
              ...
              {paymentMethod === "stripe" ? (
                ...
                <Button
                  ...
                  disabled={processing || (walletBalance !== null && walletBalance < totalAmount)}
                  className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-6 rounded-xl shadow-lg transition-all transform hover:scale-[1.02] active:scale-[0.98]"
                  size="lg"
                >
                  {processing ? (
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center justify-center gap-2">
                       <ShoppingCart className="w-5 h-5" />
                       <span>{totalAmount} $ Cüzdan Bakiyesi ile Öde</span>
                    </div>
                  )}
                </Button>
              ) : (
