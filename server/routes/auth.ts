...
router.get(
  ...
  async (req: any, res) => {
    try {
      ...
      const leftStats = (user as any).leftChild
        ? await mongoDb.getDetailedLegStats((user as any).leftChild)
        : null;
      const rightStats = (user as any).rightChild
        ? await mongoDb.getDetailedLegStats((user as any).rightChild)
        : null;
      ...
      return res.json({
        ...
        analysis: {
          user: {
            ...
            name: user.fullName,
            memberId: user.memberId,
            careerLevel: (user.careerLevel as any).name,
          },
          leftLeg: leftStats,
          rightLeg: rightStats,
...
router.get("/me", requireAuth, async (req: any, res) => {
  try {
    ...
    // Self-healing fallback to sync activeUntil and membershipEndDate
    const updatedUser = { ...user };
    if (user.membershipEndDate && !user.activeUntil) {
      updatedUser.activeUntil = user.membershipEndDate;
      await mongoDb.updateUser(userId, { activeUntil: user.membershipEndDate }).catch(() => {});
    } else if (user.activeUntil && !user.membershipEndDate) {
      updatedUser.membershipEndDate = user.activeUntil;
      await mongoDb.updateUser(userId, { membershipEndDate: user.activeUntil }).catch(() => {});
    }
...
// GET /api/auth/member/pool-stats - Real-time Weekly Performance Pool stats for user
router.get("/member/pool-stats", requireAuth, async (req: any, res) => {
  try {
    const userId = req.userId;
    const user = await mongoDb.getUserById(userId);
    if (!user) {
    ...
    const totalPoolAmount = await mongoDb.getPassiveIncomePoolAmount();
    
    // Calculate monthly sales in the last 30 days
    const { ProductPurchase } = await import('../lib/models');
    ...
    // Calculate user monthly sales
    const userSalesPurchases = monthlyPurchases.filter(p => p.referralCode === user.memberId || p.referralCode === user.referralCode);
    const userWeeklySales = userSalesPurchases.reduce((sum, p) => sum + (p.totalAmount || 0), 0);
    const userWeeklySalesCount = userSalesPurchases.length;

    // Calculate total monthly sales of ALL eligible users (active users who have >= 1 sale)
    const allUsers = await mongoDb.getAllUsers();
    const usersMonthlySales = allUsers.map(u => {
      const uSales = monthlyPurchases
        .filter(p => p.referralCode === u.memberId || p.referralCode === u.referralCode)
        .reduce((sum, p) => sum + (p.totalAmount || 0), 0);
      
      const uCount = monthlyPurchases
        .filter(p => p.referralCode === u.memberId || p.referralCode === u.referralCode)
        .length;
    ...
    const eligibleUsers = usersMonthlySales.filter(u => u.isActive && u.salesCount >= 1 && u.salesAmount > 0);
    const totalWeeklySales = eligibleUsers.reduce((sum, u) => sum + u.salesAmount, 0);

    let estimatedShare = 0;
    const userIsEligible = user.isActive && userWeeklySalesCount >= 1;
    if (userIsEligible && totalWeeklySales > 0) {
      estimatedShare = (userWeeklySales / totalWeeklySales) * totalPoolAmount;
    }
  ...
  try {
    ...
    return res.json({
      ...
      users: usersWithActivity.map((user) => ({
        ...
        activityStats: (user as any).activityStats,
  ...
  try {
    ...
    if (isActive === true) {
      ...
      const careerLevel = getCareerLevel({
        ...
        legCirosTL: user.legCirosTL ? Object.fromEntries(user.legCirosTL as any) : {}
      });
      ...
      (user as any).activeUntil = newEndDate;
      
      (user as any).approvedAt = new Date();
      user.receiptVerified = true;
      user.careerLevel = {
        id: (careerDetails.order || 0).toString(),
        name: careerDetails.name,
        displayName: careerDetails.displayName || careerDetails.name,
        level: careerDetails.order || 0,
        isActive: true,
        commissionRate: careerDetails.bonusPercent,
        passiveIncomeRate: careerDetails.depthBonusPercent || 0,
        requiredTeamCiroTL: careerDetails.requiredTeamCiroTL || 0,
        requiredUSD: careerDetails.requiredUSD,
        requiredActivePeople: careerDetails.requiredActivePeople || 0,
      ...
      try {
        ...
        if (commissionResult.transactions.length > 0) {
          ...
          try {
            ...
            for (const tx of commissionResult.transactions as any[]) {
...
// ==========================================
// SYSTEM UPDATE MODE (MAINTENANCE) STATE & ENDPOINTS
// ==========================================

let isSystemUpdating = false;
let systemUpdateCompletedMessage = false;

// Async function to load initial state from database once connected
const loadInitialUpdateState = async () => {
  try {
    const settings = await mongoDb.getSettings();
    if (settings && settings.isSystemUpdating !== undefined) {
      isSystemUpdating = !!settings.isSystemUpdating;
    }
  } catch (err) {
    // Database might not be initialized yet, retry in 10 seconds
    setTimeout(loadInitialUpdateState, 10000);
  }
};
// Start loading initial state
setTimeout(loadInitialUpdateState, 5000);

// Public GET status
router.get("/system-update-status", async (req: any, res: any) => {
  return res.json({
    success: true,
    isSystemUpdating,
    systemUpdateCompletedMessage
  });
});

// Admin POST toggle
router.post("/admin/system-update-toggle", requireAdmin, async (req: any, res: any) => {
  try {
    const { status } = req.body;
    if (status === true) {
      isSystemUpdating = true;
      systemUpdateCompletedMessage = false;
    } else {
      isSystemUpdating = false;
      systemUpdateCompletedMessage = true;
      // After 3 seconds, turn off the completed message too so the app continues
      setTimeout(() => {
        systemUpdateCompletedMessage = false;
      }, 3000);
    }
    ...
    try {
      await mongoDb.updateSystemSetting("isSystemUpdating", isSystemUpdating);
    } catch (dbErr) {
      console.error("Database settings save error:", dbErr);
    }
    ...
    return res.json({
      ...
      isSystemUpdating,
      systemUpdateCompletedMessage,
      message: isSystemUpdating ? "Sistem güncelleniyor moduna alındı." : "Sistem güncellemesi tamamlandı."
    });
  } catch (error) {
    const err = error as Error;
    return res.status(500).json({ success: false, error: err.message || "Güncelleme modu değiştirilemedi." });
  }
});
