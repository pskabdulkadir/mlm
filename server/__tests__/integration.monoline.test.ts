...
describe('Monoline Commission System Integration', () => {
  ...
  beforeEach(async () => {
    ...
    try {
      ...
      // Ignored: directory might not exist yet
    }

    // 2. Clear mlmDb in-memory structures
    mlmDb.userIds = [];
    mlmDb.indices = {
      referral: {},
      phone: {},
    ...
    // 4. Clear MongoDB test users to avoid E11000 duplicate key error
    if (mongoose.connection.readyState !== 0) {
      try {
        const UserMongoose = mongoose.model('User');
        await UserMongoose.deleteMany({ email: /@test\.com$/ });
        
        try {
          await mongoose.model('WalletTransaction').deleteMany({});
        } catch (e) {
          // Model might not be loaded yet
        }
        try {
          await mongoose.model('PassiveIncomePool').deleteMany({});
        } catch (e) {
          // Model might not be loaded yet
        }
        try {
          await mongoose.model('CommissionLog').deleteMany({});
        } catch (e) {
          // Model might not be loaded yet
        }
      } catch (err) {
        console.warn("Could not clear MongoDB test data:", err);
      }
    }
