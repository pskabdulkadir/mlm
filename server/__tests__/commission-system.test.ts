...
import { User, WalletTransaction, CommissionLog, PassiveIncomePool, DistributionLock } from '../lib/models';
...
const createTestUser = async (overrides: any = {}) => {
  const userData = {
    ...
    memberId: `ak${String(Math.floor(Math.random() * 900000) + 100000)}`,
    wallet: {
      balance: 0,
      totalEarnings: 0,
      sponsorBonus: 0,
...
describe('Commission System Tests', () => {
  ...
  describe('Commission Calculation', () => {
    ...
    it('should calculate depth commissions correctly', async () => {
      ...
      const user1 = await createTestUser({ 
        ...
        careerLevel: { id: '3', name: 'Mutmainne', level: 3, commissionRate: 4, displayName: 'Nefs-i Mutmainne' }
      });
      const user2 = await createTestUser({ 
        email: 'user2@test.com', 
        sponsorId: user1.id,
        careerLevel: { id: '2', name: 'Mülhime', level: 2, commissionRate: 3, displayName: 'Nefs-i Mülhime' }
      });
      const user3 = await createTestUser({ 
        email: 'user3@test.com', 
        sponsorId: user2.id,
        careerLevel: { id: '1', name: 'Nefs-i Emmare', level: 1, commissionRate: 2, displayName: 'Nefs-i Emmare' }
      });
      const buyer = await createTestUser({ 
        email: 'buyer@test.com', 
        sponsorId: user3.id 
  ...
  describe('Passive Income Distribution', () => {
    ...
    it('should handle weighted by career level distribution', async () => {
      const user1 = await createTestUser({ 
        email: 'user1@test.com',
        careerLevel: { id: '1', level: 1, name: 'Level 1', displayName: 'Level 1' }
      });
      const user2 = await createTestUser({ 
        email: 'user2@test.com',
        careerLevel: { id: '2', level: 2, name: 'Level 2', displayName: 'Level 2' }
      });
