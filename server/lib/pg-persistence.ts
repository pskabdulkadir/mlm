...
export const pgPersistence = {
  ...
  async clearAllExceptAdmin(adminId: string): Promise<void> {
    if (!this.enabled) return;
    try {
      const db = getPool()!;
      await db.query('DELETE FROM kz_users WHERE id <> $1', [adminId]);
      await db.query('DELETE FROM kz_wallet_transactions');
      await db.query('DELETE FROM kz_payment_requests');
      await db.query('DELETE FROM kz_clone_pages WHERE user_id <> $1', [adminId]);
      console.log('✅ PostgreSQL tables cleared (except admin user and products/misc)');
    } catch (err) {
      console.error('pgPersistence.clearAllExceptAdmin error:', err);
    }
  }
};
