...
const UserSchema = new Schema<IUserDocument>({
  ...
  membershipType: { type: String, enum: ["entry", "monthly", "yearly", "free", "standard", "elite", "vip", "premium", "NONE"], default: "entry" },
  ...
  career_level: { type: Number, default: 1 },
  total_team_ciro: { type: Number, default: 0 },
  direct_references: { type: Number, default: 0 },
  heir_id: { type: String },
  inheritance_status: { type: String, default: "none" },
  death_certificate_verified: { type: Boolean, default: false },
  legacyHeir: {
    fullName: String,
    email: String,
    phone: String,
  ...
  originalSponsorId: { type: String },
  isBurnoutActive: { type: Boolean, default: false },
  blueprintSettings: {
    donationRate: { type: Number, default: 0 },
    refusalsCount: { type: Number, default: 0 },
    refusalTitle: { type: String, default: "Acemi Satıcı" },
    timeCredits: { type: Number, default: 3 }
  },
  activity_score: { type: Number, default: 100 },
  isDeadAccount: { type: Boolean, default: false },
});
...
// --- DistributionLock ---
const DistributionLockSchema = new Schema({
...
// Export all models
export const DistributionLock = mongoose.models.DistributionLock || mongoose.model('DistributionLock', DistributionLockSchema);
...
// --- MonthlySummary for Read-Only Projections / Summaries ---
const MonthlySummarySchema = new Schema({
  userId: { type: String, required: true, index: true },
  yearMonth: { type: String, required: true, index: true }, // "YYYY-MM"
  personalSalesVolume: { type: Number, default: 0 },
  isEligibleForPoolsAndBonuses: { type: Boolean, default: false },
  careerEarnings: { type: Number, default: 0 },
  poolEarnings: { type: Number, default: 0 },
  status: { type: String, enum: ['PAID', 'HELD', 'NONE'], default: 'NONE' },
  updatedAt: { type: Date, default: Date.now }
});
MonthlySummarySchema.index({ userId: 1, yearMonth: 1 }, { unique: true });

export const MonthlySummary = mongoose.models.MonthlySummary || mongoose.model('MonthlySummary', MonthlySummarySchema);

// --- BlueprintRequest ---
const BlueprintRequestSchema = new Schema({
  id: { type: String, required: true, unique: true },
  userId: { type: String, required: true, index: true },
  userName: { type: String, required: true },
  userEmail: { type: String },
  type: { type: String, required: true }, // "insurance-grant" | "kardeslik-aid" | "assign-heir" | "trigger-inheritance" | "toggle-burnout"
