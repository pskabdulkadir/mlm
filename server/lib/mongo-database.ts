...
export const mongoDb = {
  async init() {
    try {
      ...
      // Create and ensure admin user has up-to-date settings and password
  ...
  async createSeedData() {
    ...
    if (!adminUser) {
      ...
      const updatedAdmin = await User.findOne({ email: "psikologabdulkadirkan@gmail.com" });
      if (updatedAdmin) {
        pgPersistence.saveUser(updatedAdmin.toObject()).catch(() => {});
      }
  ...
  async wipeDatabase() {
    console.log("🧹 [SYSTEM CLEAR] Initiating unconditional system reset to start FROM ADMIN ONLY for production...");
    
    // Find or ensure admin user exists first
    let adminUser = await User.findOne({ email: "psikologabdulkadirkan@gmail.com" });
    if (!adminUser) {
      ...
      // Reset admin's stats to 0, level to Nefs-i Emmare (level 0), and ensure active status
      await User.updateOne(
        { email: "psikologabdulkadirkan@gmail.com" },
        {
          $set: {
    ...
    // Now delete all other users
    const deleteResult = await User.deleteMany({
      email: { $ne: "psikologabdulkadirkan@gmail.com" }
    });
    console.log(`🗑️ Deleted non-admin users: ${deleteResult.deletedCount}`);

    // Clean up all related/transactional collections completely
    const collectionsToClear = [
      ...
      Notification,
      ZoomTraining,
      mongoose.models.CommissionAudit || null,
      mongoose.models.CommissionLog || null,
      mongoose.models.PassiveIncomePool || null,
      mongoose.models.CompanyFund || null,
      ClonePage
    ].filter(Boolean);
    ...
    // Reset sequence Counters so registration starts perfectly from sequence 2 for the first user
    await Counter.updateOne({ _id: "memberId" }, { $set: { seq: 1 } }, { upsert: true });
    await Counter.updateOne({ _id: "globalRank" }, { $set: { seq: 1, lastUserId: adminUser.id } }, { upsert: true });

    // Also clear PostgreSQL if enabled
    if (pgPersistence.enabled) {
      await pgPersistence.clearAllExceptAdmin(adminUser.id);
    }

    console.log("🔄 Counters reset: next dynamic registration will be ak000002 with globalRank 2.");
    return { success: true, deletedUsers: deleteResult.deletedCount };
  },
  ...
  async clearAllUsers() {
    ...
    // Clear related data from Postgres persistence
    if (pgPersistence.enabled && admin) {
      await pgPersistence.clearAllExceptAdmin(admin.id);
    }
    ...
    // Clean up local filesystem sharding folders inside data/users to avoid restoring old records
    try {
      const fsPromise = await import("fs/promises");
      const pathModule = await import("path");
      const usersDirPath = pathModule.join(process.cwd(), "data", "users");
      const indexDirPath = pathModule.join(process.cwd(), "data", "index");
      await fsPromise.rm(usersDirPath, { recursive: true, force: true }).catch(() => {});
      await fsPromise.mkdir(usersDirPath, { recursive: true }).catch(() => {});
      await fsPromise.rm(indexDirPath, { recursive: true, force: true }).catch(() => {});
      await fsPromise.mkdir(indexDirPath, { recursive: true }).catch(() => {});
      console.log("🧹 Filesystem data shards cleared successfully.");
    } catch (fsErr) {
      console.error("⚠️ Error clearing legacy shard folders:", fsErr);
    }

    // Synchronize Counter sequence
    await this.synchronizeMemberIdCounter();
  ...
  async migrateMemberIds() {
    ...
    await Counter.updateOne({ _id: "memberId" }, { $set: { seq: updatedCount } }, { upsert: true });
  ...
  // Logs & Activities
  async createMemberLog(data: { userId: string; action: string; details?: any; [key: string]: any }) {
  ...
  async createMemberActivity(data: { userId: string; activityType: string; details?: any; [key: string]: any }) {
  ...
  async createAdminLog(data: { adminId: string; action: string; targetUserId?: string; details?: any; [key: string]: any }) {
  ...
  async createProductPurchase(data: {
    ...
    // Activate the user immediately upon purchasing any product
    await User.updateOne({ id: data.userId }, { $set: { isActive: true } });

    const purchaseAmount = data.totalAmount || 0;
    if (purchaseAmount > 0) {
      try {
        ...
        try {
          ...
          const buyerUser = await MongoUser.findOne({ id: data.userId });
        ...
        console.error("Error updating career status in createProductPurchase:", err);
      }
    }
    ...
    try {
      ...
      console.error("Error running releaseHeldCommissionsForEligibleUsers after purchase:", err);
    }

    try {
      // Invalidate Redis/Memory caches for the user
      const { getCacheManager } = await import("../config/cache.config");
      const cache = await getCacheManager();
      await cache.delete(`user:eligibility:${data.userId}`);
      await cache.delete(`user:challenges:${data.userId}`);

      // Also invalidate for the sponsor if they have one
      const buyerUser = await User.findOne({ id: data.userId });
      if (buyerUser && buyerUser.sponsorId) {
        await cache.delete(`user:eligibility:${buyerUser.sponsorId}`);
        await cache.delete(`user:challenges:${buyerUser.sponsorId}`);
      }
    } catch (err) {
      console.error("Error invalidating cache after purchase:", err);
    }
  ...
  async getSocialMediaLinks() {
    const setting = await SystemSettings.findOne({ key: "social_media_links" }).lean() as any;
  ...
  async getSpiritualContent() {
    const setting = await SystemSettings.findOne({ key: "spiritual_content" }).lean() as any;
    if (!setting?.value) {
      return {
        hadiths: [],
        sunnahs: [],
  ...
  async deleteTestUsers() {
    console.log("🧹 Starting DEEP PRODUCTION RESET (Go-Live Purge)...");

    // 1. Identify users to preserve
    const adminEmails = ["psikologabdulkadirkan@gmail.com", "programaktif@gmail.com"];
    
    let usersToPreserve: any[] = [];
    try {
      usersToPreserve = await User.find({
        $or: [
      ...
      console.error("⚠️ Error finding users to preserve:", findErr);
    }

    const preserveIds = usersToPreserve.map(u => u.id).filter(Boolean);

    if (preserveIds.length === 0) {
      console.warn("⚠️ No admin users or ak000001 to preserve, aborting deep reset to prevent bricking the DB.");
      return 0;
    }
    ...
    try {
      ...
      console.log(`🗑️ Deleted ${deletedCount} test/trial users.`);
    } catch (delErr) {
      console.error("⚠️ Error deleting non-preserved users:", delErr);
    }

    // 3. Reset stats, wallets, referrals and levels of preserved key users
    for (const u of usersToPreserve) {
      const isRootAdmin = u.email === "psikologabdulkadirkan@gmail.com" || u.memberId === "ak000001";
      try {
        await User.updateOne(
          { id: u.id },
          {
            $set: {
              isActive: true,
              sponsorId: isRootAdmin ? null : "ak000001",
              directReferrals: 0,
              leftLegCount: 0,
              rightLegCount: 0,
              unilevelCiro: 0,
        ...
        // Update PostgreSQL persistence layer if enabled
        const updatedUser = await User.findOne({ id: u.id });
        if (updatedUser) {
          pgPersistence.saveUser(updatedUser.toObject()).catch(() => {});
        }
      } catch (userUpErr) {
        console.error(`⚠️ Error resetting user stats for ${u.email}:`, userUpErr);
      }
    }
    console.log("🔄 Reset wallet balances and stats of root/admin users.");

    // Helper to safely delete collections
    const safeDelete = async (model: any, name: string) => {
      try {
        if (model && typeof model.deleteMany === 'function') {
          await model.deleteMany({});
          console.log(`🧹 Collection cleared: ${name}`);
        }
      } catch (err) {
        console.error(`⚠️ Error clearing collection ${name}:`, err);
      }
    };

    // 4. Clean up all transactional and related collections
    await safeDelete(PendingPlacement, "PendingPlacement");
    await safeDelete(WalletTransaction, "WalletTransaction");
    await safeDelete(MonolineCommission, "MonolineCommission");
    await safeDelete(ProductPurchase, "ProductPurchase");
    await safeDelete(PaymentRequest, "PaymentRequest");
    await safeDelete(RealTimeTransaction, "RealTimeTransaction");
    await safeDelete(MemberSession, "MemberSession");
    await safeDelete(HatimProgress, "HatimProgress");
    await safeDelete(DuaRequest, "DuaRequest");
    await safeDelete(MemberLog, "MemberLog");
    await safeDelete(MemberActivity, "MemberActivity");
    await safeDelete(AdminLog, "AdminLog");

    try {
      if (mongoose.models.CommissionAudit) {
        await mongoose.models.CommissionAudit.deleteMany({});
      }
    } catch (e) {
      console.error("⚠️ Error clearing CommissionAudit:", e);
    }

    try {
      if (mongoose.models.CommissionLog) {
        await mongoose.models.CommissionLog.deleteMany({});
      }
    } catch (e) {
      console.error("⚠️ Error clearing CommissionLog:", e);
    }

    try {
      await ClonePage.deleteMany({ userId: { $not: { $in: preserveIds } } });
      console.log("🧹 Clone pages cleared.");
    } catch (e) {
      console.error("⚠️ Error clearing ClonePages:", e);
    }

    // 5. Reset sequence Counters so registration starts perfectly from sequence 2 for first user
    try {
      await Counter.updateOne({ _id: "memberId" }, { $set: { seq: 1 } }, { upsert: true });
      await Counter.updateOne({ _id: "globalRank" }, { $set: { seq: 1, lastUserId: preserveIds[0] || "" } }, { upsert: true });
      console.log("🔄 Sequence counters reset: next dynamic registration will be ak000002 with globalRank 2.");
    } catch (cntErr) {
      console.error("⚠️ Error resetting sequence counters:", cntErr);
    }

    // 6. Reset passive income pool
    try {
      await PassiveIncomePool.updateOne({}, { $set: { totalAmount: 0 } }, { upsert: true });
      console.log("🌊 Passive pool amount reset to 0.");
    } catch (poolErr) {
      console.error("⚠️ Error resetting passive pool:", poolErr);
    }
  ...
  async getHatimProgress(userId: string) {
    let progress: any = await HatimProgress.findOne({ userId }).lean();
    if (!progress) {
  ...
  async getPromotionSettings() {
    const setting = await SystemSettings.findOne({ key: "promotions" }).lean() as any;
    return setting?.value || [];
  },
  ...
  async getGiftSettings() {
    const setting = await SystemSettings.findOne({ key: "gift_settings" }).lean() as any;
    return setting?.value || {
      welcomeBonus: 0,
      firstDepositBonus: 0,
      achievementRewards: [],
  ...
  async processDepositRequest(id: string, adminId: string, action: string, note: string) {
    ...
    if (action === 'approve') {
      ...
      if (descLower.includes('yearly') || descLower.includes('yıllık')) {
        activeMonths = 12;
      } else if (descLower.includes('monthly') || descLower.includes('aylık')) {
        activeMonths = 1;
      } else if (descLower.includes('entry') || descLower.includes('giriş')) {
        activeMonths = 1;
      } else {
  ...
  async peekNextMemberId(): Promise<string> {
    try {
      const allUsers = await User.find({ memberId: /^ak\d+$/ }, { memberId: 1 }).lean() as any[];
      ...
      return `ak${String(nextNum).padStart(6, '0')}`;
    } catch (error) {
      console.error("Error peeking next member ID:", error);
      return "ak000002";
    }
  },
  ...
  async getNextMemberId(): Promise<string> {
    try {
      ...
      return `ak${String(counter.seq).padStart(6, '0')}`;
    }
  },
  ...
  async enhancedAutoPlacement(userId: string, sponsorId: string, preferences: any) {
    return {
      placedUserId: userId,
      position: preferences.position || "left",
      parentId: sponsorId,
      depth: 1,
      path: [sponsorId]
    };
  ...
  async getDetailedLegStats(childId: string) {
    return {
      totalUsers: 0,
      activeUsers: 0,
      volume: 0,
  ...
  async findAvailablePositions(userId: string, maxDepth: number = 7) {
    return [
      { userId, position: "left", parentId: userId, depth: 1 },
      { userId, position: "right", parentId: userId, depth: 1 }
    ];
  },
