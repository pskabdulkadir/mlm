  ...
  db: {
    read: async () => {
      try {
        ...
        mlmDb.db.data = { users: [], products: [], transactions: [], settings: {} } as any;
      }
    },
    write: async () => {
      try {
...
async function getUserByIdInternal(id: string) {
  ...
  // Try fetching from MongoDB first to ensure absolute real-time accuracy and synchronization
  try {
    if (mongoose.connection.readyState === 1) {
      const { User: MongoUser } = await import("./models");
      const mongoUser = await MongoUser.findOne({ id }).lean();
      if (mongoUser) {
        const mappedUser = { ...mongoUser, id: mongoUser.id || mongoUser._id.toString() };
        await writeUserFile(id, mappedUser);
        return mappedUser;
      }
    }
  } catch (mongoErr) {
    console.warn('MongoDB read error in getUserByIdInternal:', mongoErr);
  }

  // Fallback to index lookup in filesystem
  try {
